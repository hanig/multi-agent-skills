...
class DispatchBaseGuardTests(unittest.TestCase):
    ...
    def test_equal_target_does_not_require_remote_fetch(self):
        head = git(self.repo, "rev-parse", "HEAD")
        real_route = S._git_push_destination

        def fetch_rejecting_route(repo, raw, resolved, remote_index, *args,
                                  timeout=60):
            if args and args[0] == "fetch":
                return 1, "", "upload-pack is unavailable"
            return real_route(repo, raw, resolved, remote_index, *args,
                              timeout=timeout)

        with mock.patch.object(
                S, "_git_push_destination",
                side_effect=fetch_rejecting_route):
            rc, output = self.run_dispatch("equal-without-fetch")

        self.assertEqual(rc, 0, output)
        self.assertIn(head, output)
        self.assertIn("from branch 'main'", output)

    def test_ahead_target_with_local_remote_tip_does_not_require_fetch(self):
        remote_head = git(self.repo, "rev-parse", "HEAD")
        local_head = self.commit("honest-local-ahead")
        real_route = S._git_push_destination
        ...
        with mock.patch.object(
                S, "_git_push_destination",
                side_effect=fetch_rejecting_route):
            rc, output = self.run_dispatch("ahead-with-local-target")
    ...
    def test_ambiguous_successful_attempt_ref_lookup_refuses(self):
        real_route = S._git_push_destination

        def duplicate_attempt_ref(repo, raw, resolved, remote_index, *args,
                                  timeout=60):
            ref = args[-1] if args else ""
            if (args and args[0] == "ls-remote"
                    and str(ref).startswith("refs/heads/swarm-")):
        ...
        with mock.patch.object(
                S, "_git_push_destination",
                side_effect=duplicate_attempt_ref):
            rc, output = self.run_dispatch("ambiguous-attempt-ref")

        self.assertNotEqual(rc, 0, output)
        self.assertIn("no single valid exact-ref answer", output)
        self.assertNotIn("submitted", output)

    def test_attempt_ref_status_two_with_listing_refuses(self):
        real_route = S._git_push_destination

        def contradictory_attempt_ref(repo, raw, resolved, remote_index,
                                      *args, timeout=60):
            ref = args[-1] if args else ""
            if (args and args[0] == "ls-remote"
                    and str(ref).startswith("refs/heads/swarm-")):
        ...
        with mock.patch.object(
                S, "_git_push_destination",
                side_effect=contradictory_attempt_ref):
            rc, output = self.run_dispatch("contradictory-attempt-ref")

        self.assertNotEqual(rc, 0, output)
        self.assertIn("contradictory", output)
        self.assertNotIn("submitted", output)
    ...
    def test_shallow_checkout_can_deepen_from_anchored_push_route(self):
        ...
        git(self.repo, "push", "-q", str(push_remote),
            ancestor + ":refs/heads/main")
        git(self.repo, "push", "-q", str(push_remote),
            descendant + ":refs/heads/history-source")
        git(push_remote, "symbolic-ref", "HEAD", "refs/heads/not-target")
...
class DispatchCanaryTests(unittest.TestCase):
    ...
    def test_honest_canary_wait_exits_successfully_through_cmd_run(self):
        plan_path = self.tmp / "canary-plan.json"
        plan_path.write_text(json.dumps(self.plan()))
        args = Namespace(
            plan=str(plan_path), state_dir=str(self.tmp / "command-state"),
        ...
        self.assertEqual(rc, S.EXIT_OK, output)
        self.assertIn("dispatched 1 unit(s)", output)
        self.assertIn("has not reached DONE", output)

    def test_failed_canary_with_only_dependents_exits_nonzero(self):
        plan = self.plan()
        plan["units"] = [self.unit("z-canary"), self.unit("dependent")]
        plan["units"][1]["needs"] = ["z-canary"]
        plan_path = self.tmp / "failed-canary-plan.json"
        plan_path.write_text(json.dumps(plan))
        state_dir = self.tmp / "failed-canary-state"
        S.save_state(str(state_dir), {
            ...
            "units": {
                "z-canary": {
                    ...
                    "attempts": [S.DRY_PREFIX + "/runs/z-canary/a1"],
        ...
        args = Namespace(
            ...
            root=str(self.tmp / "failed-canary-runs"), dry_run=True,
            max_new_dispatches=None, accept_plan_change=False)
        stream = io.StringIO()
        ...
        output = stream.getvalue()
        self.assertEqual(rc, S.EXIT_HALTED, output)
        self.assertIn("dependent: held", output)
        self.assertIn("canary DONE evidence is absent", output)

    def test_same_advance_canary_submit_failure_exits_nonzero(self):
        plan = self.plan()
        plan["units"] = [self.unit("z-canary"), self.unit("dependent")]
        plan["units"][1]["needs"] = ["z-canary"]
        plan_path = self.tmp / "same-advance-failure-plan.json"
        plan_path.write_text(json.dumps(plan))
        state_dir = self.tmp / "same-advance-failure-state"
        args = Namespace(
            plan=str(plan_path), state_dir=str(state_dir),
            root=str(self.tmp / "same-advance-failure-runs"), dry_run=True,
            max_new_dispatches=None, accept_plan_change=False)
        counter = {"n": 0}
        ...
        with mock.patch.object(S, "_allocate", side_effect=allocate), \
                mock.patch.object(
                    S, "_submit", return_value=(None, "submit exploded")), \
        ...
        self.assertEqual(rc, S.EXIT_HALTED, output)
        self.assertIn("submit exploded", output)
        self.assertIn("dependent: held", output)
        self.assertIn("canary DONE evidence is absent", output)
        self.assertEqual(state["units"]["z-canary"]["state"], "FAILED")
        self.assertEqual(state["units"]["dependent"]["state"], "HELD")

    def test_same_advance_canary_allocation_failure_exits_nonzero(self):
        plan = self.plan()
        plan["units"] = [self.unit("z-canary")]
        plan_path = self.tmp / "same-advance-allocation-plan.json"
        plan_path.write_text(json.dumps(plan))
        args = Namespace(
            plan=str(plan_path),
            state_dir=str(self.tmp / "same-advance-allocation-state"),
            root=str(self.tmp / "same-advance-allocation-runs"), dry_run=True,
            max_new_dispatches=None, accept_plan_change=False)
        stream = io.StringIO()

        with mock.patch.object(
                S, "_allocate", return_value=(None, "allocation exploded")), \
                contextlib.redirect_stdout(stream):
            rc = S.cmd_run(args)

        output = stream.getvalue()
        self.assertEqual(rc, S.EXIT_HALTED, output)
        self.assertIn("allocation exploded", output)
        self.assertIn("canary DONE evidence is absent", output)

    def test_canary_preflight_refusal_cannot_release_width_to_noncanary(self):
        """Luna round 3: cleared canary admission must not open width 1."""
        plan = self.plan()
        state = {"units": {}}
        counter = {"n": 0}
        ...
        refusal = S.PreflightRefusal("workspace is temporarily dirty")
        refusal.workspace = str(self.tmp)

        def submit(unit, _attempt, _dry_run, *_rest):
            if unit["id"] == "z-canary":
                return None, refusal
        ...
        self.assertEqual(state["units"]["z-canary"]["state"],
                         "PREFLIGHT_REFUSED")
        self.assertFalse(state["units"]["z-canary"]["attempts"])
        self.assertNotEqual(state["units"]["a-rest"]["state"], "SUBMITTED")
        self.assertIn("has not reached DONE", " ".join(report))

    def test_retryable_canary_cannot_exceed_full_inherited_width(self):
        plan = self.plan()
        running = self.tmp / "legacy-running-attempt"
        running.mkdir()
        state = {
            "plan_digest": S.plan_digest(plan),
            "units": {
    ...
    def test_retryable_preflight_refusal_remains_legitimate_waiting(self):
        plan = self.plan()
        plan["units"] = [{
            "id": "z-canary", "kind": "code", "repo": str(self.tmp),
            "target_branch": "main", "prompt": "work",
            "mode": "full-access",
            "outputs": ["z-canary.txt"],
        }]
        plan_path = self.tmp / "retryable-preflight-plan.json"
        plan_path.write_text(json.dumps(plan))
        state_dir = self.tmp / "retryable-preflight-state"
        args = Namespace(
            plan=str(plan_path), state_dir=str(state_dir),
            root=str(self.tmp / "retryable-preflight-runs"), dry_run=True,
            max_new_dispatches=None, accept_plan_change=False)
        attempt = self.tmp / "retryable-preflight-runs" / "z-canary" / "a1"
        ...
        stream = io.StringIO()
        with mock.patch.object(S, "_allocate", side_effect=allocate), \
                mock.patch.object(S, "_submit", return_value=(None, refusal)), \
                contextlib.redirect_stdout(stream):
            rc = S.cmd_run(args)

        output = stream.getvalue()
        state = S.load_state(str(state_dir))
        canary = state["units"]["z-canary"]
        self.assertEqual(rc, S.EXIT_OK, output)
        self.assertEqual(canary["state"], "PREFLIGHT_REFUSED")
        self.assertIsNone(canary["attempt_dir"])
        self.assertFalse(canary["attempts"])
    ...
    def test_failed_admissions_do_not_refund_width_before_done(self):
        plan = self.plan()
        plan["dispatch_canary"]["width"] = 2
        state = {"units": {}}
        ...
        self.assertEqual(dispatched, 2, report)
        self.assertEqual(state["units"]["z-canary"]["state"], "SUBMITTED")
        self.assertEqual(state["units"]["a-rest"]["state"], "SUBMITTED")
        self.assertFalse(state["units"]["b-rest"]["attempts"])

        state["units"]["z-canary"]["state"] = "FAILED"
        state["units"]["z-canary"]["attempt_dir"] = None
        state["units"]["z-canary"]["job_id"] = None
        report, dispatched, halted = self.advance(plan, state)

        self.assertEqual(dispatched, 0, report)
        self.assertIn("is FAILED, not DONE", halted)
        self.assertFalse(state["units"]["b-rest"]["attempts"])

    def test_done_opens_gate_despite_an_unclosed_old_failed_claim(self):
        plan = self.plan()
        failed_attempt = self.tmp / "failed-after-done"
        state = {
            "units": {
                "z-canary": {
                    "attempt_dir": None, "attempts": [], "job_id": None,
                    "state": "DONE", "gpu_hours": 0.0,
                },
                "a-rest": {
                    "attempt_dir": None,
                    "attempts": [str(failed_attempt)],
                    "job_id": "old-executor", "state": "FAILED",
                    "gpu_hours": 0.0,
                },
            },
        }
        ...
        with mock.patch.object(
                S, "_claim_liveness",
                return_value=(S.CLAIM_LIVE, "old executor remains")):
            report, dispatched, halted = self.advance(
                plan, state, dry_run=False)

        self.assertEqual(dispatched, 1, report)
        self.assertIsNone(halted, report)
        self.assertEqual(state["units"]["b-rest"]["state"], "SUBMITTED")

    def test_unclosed_failed_claim_counts_then_halts_at_width(self):
        plan = self.plan()
        plan["dispatch_canary"]["width"] = 3
        canary_attempt = self.tmp / "running-canary"
        failed_attempt = self.tmp / "failed-live-below-width"
        state = {
            "units": {
                "z-canary": {
                    "attempt_dir": str(canary_attempt),
                    "attempts": [str(canary_attempt)],
                    "job_id": "canary-job", "state": "RUNNING",
                    "gpu_hours": 0.0,
                },
                "a-rest": {
                    "attempt_dir": None,
        ...
        self.assertEqual(dispatched, 1, report)
        self.assertIn("capacity claim for terminal unit 'a-rest'", halted)
        self.assertEqual(state["units"]["b-rest"]["state"], "SUBMITTED")

    def test_closed_failed_claim_frees_capacity_only_for_new_canary(self):
        plan = self.plan()
        plan["dispatch_canary"]["width"] = 2
        failed_attempt = self.tmp / "failed-dry-attempt"
        state = {
            "units": {
                "a-rest": {
                    "attempt_dir": None,
                    "attempts": [S.DRY_PREFIX + str(failed_attempt)],
                    "job_id": "dry-a-rest", "state": "FAILED",
                    "gpu_hours": 0.0,
                },
            },
        }
    ...
    def test_terminal_claim_with_live_executor_refuses_nonzero(self):
        plan = self.plan()
        failed_attempt = self.tmp / "failed-live-attempt"
        state = {
            "units": {
                "a-rest": {
                    "attempt_dir": None,
                    "attempts": [str(failed_attempt)],
                    "job_id": "executor-1", "state": "FAILED",
                    "gpu_hours": 0.0,
                },
            },
        }
        ...
        with mock.patch.object(
                S, "_claim_liveness",
                return_value=(S.CLAIM_LIVE,
                              "executor registry still lists executor-1")):
            report, dispatched, halted = self.advance(plan, state)

        self.assertEqual(dispatched, 0, report)
        self.assertIn("capacity claim for terminal unit 'a-rest'", halted)
        self.assertIn("executor remains active", halted)
        self.assertIn("canary DONE evidence is absent", halted)
        self.assertFalse(state["units"]["z-canary"]["attempts"])

    def test_unknown_terminal_claim_refusal_does_not_call_it_running(self):
        plan = self.plan()
        failed_attempt = self.tmp / "failed-unknown-attempt"
        state = {
            "units": {
                "a-rest": {
                    "attempt_dir": None,
                    "attempts": [str(failed_attempt)],
                    "job_id": "executor-unknown", "state": "FAILED",
                    "gpu_hours": 0.0,
                },
            },
        }
        ...
        with mock.patch.object(
                S, "_claim_liveness",
                return_value=(S.CLAIM_UNKNOWN,
                              "executor registry could not be queried")):
            report, dispatched, halted = self.advance(plan, state)

        self.assertEqual(dispatched, 0, report)
        self.assertIn("could not establish whether an executor or lease",
                      halted)
        self.assertNotIn("still running", halted)
        self.assertFalse(state["units"]["z-canary"]["attempts"])

    def test_closed_claim_is_rechecked_if_executor_later_reappears(self):
        plan = self.plan()
        failed_attempt = self.tmp / "failed-reappearing-attempt"
        state = {
            "units": {
                "a-rest": {
                    "attempt_dir": None,
        ...
        with mock.patch.object(
                S, "_claim_liveness",
                return_value=(S.CLAIM_FREE, "registry proves it absent")):
            report, dispatched, halted = self.advance(plan, state)
        self.assertEqual(dispatched, 1, report)
        self.assertIsNone(halted, report)
        ...
        with mock.patch.object(
                S, "_claim_liveness",
                return_value=(S.CLAIM_LIVE,
                              "executor registry lists it again")):
            report, dispatched, halted = self.advance(plan, state)

        self.assertEqual(dispatched, 0, report)
        self.assertIn("active capacity would be 2 past declared width 1",
                      halted)
        self.assertIn("executor remains active", halted)
