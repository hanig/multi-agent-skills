...
def closure_panel_complete(ledger, completed, failed, truncated=False):
    """Closure authority requires every member of the frozen panel."""
...
def new_case_ledger(case_id, contract, panel):
    return {
        "schema_version": 1,
        "generation": 0,
        "case_id": case_id,
...
def prepare_case_ledger(args, panel, snapshot):
    ...
    if ledger is None:
        ledger = new_case_ledger(path.stem, contract, panel)
    else:
        ...
        if old_panel != panel:
            if not (args.new_cycle and args.authorize_panel_change):
