...
| reviewer unavailability clears or deadlocks a finding | closure records at most two attempts for the named reviewer, then uses only a frozen alternate or an owner-authorized reassignment; otherwise it returns deferred-awaiting-authority |
...
### Fixed seats and unavailable adjudicators

The fixed-panel rule freezes panel seats; it does not require an unavailable
person or provider to hold a seat forever. `--alternate REVIEWER=ALTERNATE` on
case creation preauthorizes one alternate for that seat. Closure records two
unavailable observations for the currently named adjudicator without consuming
...
`REVIEW_ADJUDICATION` with `availability_outcome` set to
`deferred-awaiting-authority` (printed as “deferred - awaiting authority”). The
worker may stop because its work and the open finding are preserved. No finding
is cleared, no closure round is consumed, and reviewer unavailability never
automatically releases the change.
