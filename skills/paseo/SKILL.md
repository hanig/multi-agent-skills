...
name: paseo
description: >-
  Delegate work to AI coding agents through Paseo: run Claude Code, Codex,
  Kimi, or pi agents in the background, in worktree-isolated workspaces, on
  cron schedules, or as loops. Use when the user asks to hand a task to
  another model or harness ("have Codex do X", "run this with Claude", "use
  pi"), spawn or background an agent, create an isolated worktree workspace,
  schedule recurring agent work, or manage running agents (send, stop,
  inspect, archive). Prerequisite read for the paseo-advisor, paseo-handoff,
  paseo-committee, and paseo-loop skills. Not for peer messaging between
  sessions or finish notifications — that is the agent-bus skill; not for
  multi-ticket sprint orchestration — that is start-a-sprint.
---
...
**`create_agent`** — required: `title`, `provider` (`claude/opus`, `codex/gpt-5.6-sol`, …), `initialPrompt`. Optional: `workspaceId`, `notifyOnFinish`, `settings`, `labels`. Returns `{ agentId, workspaceId, … }`.
...
`claude/sonnet` (default), `claude/opus` (harder reasoning), `codex/gpt-5.6-sol` (frontier coding), `claude/haiku` (tests only).
...
When the category leaves room (or the user asks for efficient routing), run
`~/.agent-bus/bin/bus models --json` and match task difficulty to the cheapest
model that clears the bar — it carries intelligence-index scores, modalities,
live codex rate-limit windows, vLLM load, and observed tok/s. See the
**agent-bus** skill.
...
## Waiting and finish notifications

Agents take time — 10–30+ minutes is routine. Favor asynchronous workflows.

**Know which surface you have.** The `notifyOnFinish` callbacks described
above only exist when the Paseo MCP server is connected to your harness. In
the current setup no harness has it — everything goes through the CLI, and
the CLI has no channel back into your session. `notifyOnFinish` shows up in
the Paseo app UI, never in your context. Do not rely on it.

In CLI-only sessions, build the notification yourself via the agent bus:

1. Launch with `paseo run -d ... --json` and note the `agentId`.
2. Immediately arm `~/.agent-bus/bin/bus await <agentId> --timeout 3600` as a
   **background task** (Kimi Code and Claude Code both inject background-task
   completion into the session). The task completing *is* your notification.
   Its output is a self-contained report: verdict (`finished` /
   `AWAITING PERMISSION` / `TIMEOUT`), the worker's last activity lines,
   token usage, and git evidence from the worker's cwd.
3. Move on to other work. On `TIMEOUT` (exit 1), re-arm the await to keep
   waiting. On `AWAITING PERMISSION`, the worker needs a human — surface it.

Don't poll `list_agents` or `paseo ls` to "check on" a running agent. Arm an
await instead. See the **agent-bus** skill for the full pattern.
...
paseo run --provider codex/gpt-5.6-sol --mode full-access --workspace <workspace-id> "<prompt>"
paseo run --provider codex/gpt-5.6-sol --mode full-access --new-workspace worktree --worktree-mode branch-off --new-branch fix-x --base main "<prompt>"
