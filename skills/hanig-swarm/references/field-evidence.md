...
requires `--accept-plan-change`. `survey.py` reports `allow_accounts`,
...
is exactly the topology a local `flock` already guarantees and does not certify
cross-node recovery.
...
available” even though `sbatch --test-only` accepts the flags. Python 3.10.12
on andromeda and chimera sets the host floor.

Partition names do not port across the three clusters:
...
`RemoteCommand`; `--test-only` start estimates were pessimistic enough to
predict 22:08 for a job that began and ended within one second; a lifted Slurm
module needs callees, imports, and constants, with
