# Declared limits <!-- declaration: placement.reference-elaboration -->

This file expands the limit index in `../SKILL.md`. The index is authoritative <!-- declaration: placement.reference-elaboration -->
for discoverability; details here do not silently add a new closed-looking <!-- declaration: placement.reference-elaboration -->
assumption.
...
at run time; those need a preflight in every allocation. <!-- declaration: limit.runtime-canary-scope -->
...
restricts writable host bind mounts of the dispatched workload. It does not <!-- declaration: limit.container-isolation-scope -->
...
a writable host `$HOME` cannot use this profile. <!-- declaration: limit.container-isolation-scope -->

## Pre-dispatch artifact basis

Three accepted costs remain:

- An attempt dispatched before artifact bases existed can never become DONE; <!-- declaration: limit.pre-dispatch-artifact-basis -->
...
- Byte-identical regeneration and no production are observationally identical.
  In practice an allocated root should be empty; this matters when something <!-- declaration: limit.pre-dispatch-artifact-basis -->
...
an authority descriptor. Modes, Git locks, and descriptor discipline cannot <!-- declaration: limit.same-uid-authority -->
defend against the same principal. Separate Unix identities or an equivalent
OS boundary are needed. The intended threat model covers careless workers and <!-- declaration: limit.same-uid-authority -->
...
it does not establish quiescence. Accepted receipt provenance records this. <!-- declaration: limit.process-tree-quiescence -->
...
Corpus protection covers only exact regular files named by policy. Imported <!-- declaration: limit.verifier-corpus-boundary -->
...
Candidate construction never fetches. Both commits must exist locally, have a <!-- declaration: limit.integration-topology -->
unique merge base, merge without conflict, and yield a tree change. Admission
rejects a target that moved since the receipt. Squash/merge topology must <!-- declaration: limit.integration-topology -->
unambiguously expose the pre-merge target; rebase results cannot do so when <!-- declaration: limit.integration-topology -->
...
They cannot make Git metadata immutable: HEAD, index, refs, and objects must <!-- declaration: limit.worktree-inode -->
...
adoption. The post-check full Git identity match fails closed; only a Paseo <!-- declaration: limit.worktree-adoption -->
...
cleanup/archive bookkeeping only. Repo, base, branch, path, and Git identity <!-- declaration: limit.workspace-id -->
...
establish only engine termination plus final declared artifacts at its isolated <!-- declaration: limit.pipeline-interior -->
publish boundary. It cannot certify which internal step made an intermediate. <!-- declaration: limit.pipeline-interior -->
...
notifying the process. Trials exercised only same-node concurrency. Cross-node <!-- declaration: limit.coordinator-lock-topology -->
...
because writers outlive `advance`. It is shared only within one run root. Two <!-- declaration: limit.output-claim-registry -->
different `--root` values do not arbitrate. Foreign release requires positive <!-- declaration: limit.output-claim-registry -->
scheduler/Paseo absence; missing, failed, or inapplicable liveness is unknown
and refuses, potentially requiring an operator to remove a stale named claim. <!-- declaration: limit.output-claim-registry -->

## Base-branch comparison

Testing the recorded base and feature head independently attributes a
regression to the feature change. It cannot detect semantic conflict with a <!-- declaration: limit.base-branch-comparison -->
target branch that changes later. Only a test of the actual candidate/merged <!-- declaration: limit.base-branch-comparison -->
