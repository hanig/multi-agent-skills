...
def _capture_code_launch(unit_dir, u):
    ...
    # ls-remote observes the destination, while fetch makes that exact object
    # available for the ancestry decision. Comparing only a stale
    # refs/remotes/origin/* ref recreates the incident this guard closes.
    fetch_rc, _fetch_out, fetch_err = _git_push_destination(
        repo, remote_raw, remote, 4, "fetch", "--quiet", "--no-tags",
        "--recurse-submodules=no",
        remote_raw,
        f"refs/heads/{target}", timeout=120)
    if fetch_rc != 0:
        return _dispatch_base_refusal(
            u.get("id"), repo,
    ...
    if head != target_head:
        ...
        if relation in ("diverged", "unknown"):
            ...
            if shallow_rc == 0 and shallow_out == "true":
                # Fetching the push target makes that commit available but
                # does not cross the checkout's shallow boundary. Deepen from
                # the repository's fetch route solely to recover immutable
                # parent objects, then decide again against the independently
                # observed push target. Without this, a true shallow-ahead
                # checkout is misclassified as diverged.
                deepen_rc, _deepen_out, deepen_err = _git(
                    repo, "fetch", "--quiet", "--unshallow", "--no-tags",
                    "--recurse-submodules=no", "origin", target_ref,
                    timeout=120)
                if deepen_rc != 0:
                    return _dispatch_base_refusal(
                        u.get("id"), repo,
                        f"cannot establish ancestry for shallow checkout "
                        f"{base_branch!r} at {head}: {deepen_err[:200]}. "
    ...
    if remote_rc == 0 and existing_head is not None:
