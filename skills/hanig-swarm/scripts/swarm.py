    ...
    for uid in dispatch_order:
        ...
        if canary_id and not canary_done:
            ...
            if uid != canary_id and canary_disqualified:
                transient_halt = (
                    f"dispatch canary {canary_id!r} is {canary_state}, not "
                    f"DONE; refusing further fan-out because only DONE "
                    f"qualifies the canary")
                report.append(f"{uid}: waiting -- {transient_halt}")
                continue
            if uid not in admitted and len(admitted) >= canary_width:
                wait_reason = (
