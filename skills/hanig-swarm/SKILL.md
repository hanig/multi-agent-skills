...
- **`code.default-agent`:** A `code` unit defaults to `codex/gpt-5.6-sol` with `thinking: high`; provider overrides must be declared on the unit.
...
The only excluded worktree object is the exact `.git` pointer whose target
matches the coordinator-recorded linked Git directory. A replaced `.git` file,
