...
| Pi | `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills`; the override replaces Pi's user config root. Its automatic shared root is `$HOME/.agents/skills`. The native harness accepts the old `@mariozechner` and current `@earendil-works` package scopes. | `pi --version` (unverified native invocation) | [validated 0.73.1 manifest](https://registry.npmjs.org/@mariozechner/pi-coding-agent/0.73.1), [current renamed package](https://registry.npmjs.org/@earendil-works/pi-coding-agent), [0.73.1 skills source](https://github.com/badlogic/pi-mono/blob/v0.73.1/packages/coding-agent/docs/skills.md) |
