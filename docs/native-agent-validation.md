...
For dual-scope installations, the package containing the resolved `pi`
executable is tried before unrelated global packages; scope-major ordering is
only the fallback among unrelated candidates. Pi 0.73.1 may omit the
`diagnostics` key when `loader.diagnostics` is undefined. An omitted or empty
value is accepted, while a present non-empty value fails discovery.
