...
`skills/` holds two populations, and the `hanig-` prefix is the classifier both `install.sh` and `bin/doctor` read off the tree. Six authored bundles (`hanig-project`, `hanig-swarm`, `hanig-verified-workflow`, `hanig-review-gate`, `hanig-portable-handoff`, `hanig-orchestrate`) are ours. `hanig-orchestrate` is the orchestrator's operating mode and deliberately does NOT restate the grant: authority comes from the current `docs/orchestrator-mandate.md`, and a skill that copied those lists would go stale the way this file's `--escalate` line did. Eight vendored bundles (`paseo*`, `pi-fleet`, `agent-bus`, `start-a-sprint`) are upstream verbatim and `--uninstall` leaves them alone.
...
Do not report a change complete, or assert that code works, until `hanig-review-gate` has run and passed. Exit 2 (`REVIEW_UNAVAILABLE`) and 3 (`REVIEW_PARTIAL`) are not a pass; if the gate cannot run, the change is unreviewed and must be described that way. The provider keys are exported from `~/.zshrc`, which a non-interactive shell does not source, so invoke through `zsh -ic` or export them explicitly. Reproduce a finding before acting on it, and keep an author off the panel reviewing its own work. `--escalate` walks `fast` → `standard` → `deep`, each tier adding only the reviewers the previous one did not run, and the first failing tier ends it. `kimi-k3` is disabled. `sol` is enabled and `deep`-only: it took `astra`'s seat on 2026-09-24, when astra became the code driver, because an author does not review itself. The same reasoning stops a change sol authored before that date at `standard`, and only the roster enforces either, since neither `review.py` nor `committee.py` takes an author (ARC-755). So escalating does buy a reviewer — and buys it only when the cheaper tiers find nothing, which is why a `REVIEW_FAIL` at `fast` never reaches sol. Read the `enabled` flags and `profiles` lists in `reviewers.json` rather than this sentence; an earlier version of it said escalation bought nothing, which sent sessions past the one reviewer that had been added for them.

Never edit a vendored skill document. A needed correction goes in `docs/upstream-*.md` plus a test that pins the behavior, the way the `bin/bus` patch is pinned; routing decisions go in `~/.paseo/orchestration-preferences.json` (example in `examples/`). `tests/test_skill_capabilities.py::test_vendored_payload_matches_offline_manifest` hashes all eight vendored bundles and the explicit ARC-270 `bin/bus` exception from `docs/upstream-manifest.json`, without Git or a subprocess. The manifest is shipped data, not authority: a matching digest records this repository's bytes and does not prove upstream provenance. `test_vendored_sources_are_not_rewritten_on_this_branch` remains a separate, optional comparison when `origin/main` exists; it can catch a branch that rebaselines both a vendored file and the manifest, but agreement with that moving ref is still only a guard against editing here.
...
**Preserve a blocked attempt's work before any cleanup path runs.** Of two attempts blocked the same way on one day, one wrote a digested patch and one deleted its implementation "to leave the worktree clean"; nothing in the protocol decided which. Preservation must not be agent discretion: cleanup depends on a completed, restore-checked snapshot carrying base identity, content digest and validation result, and a failed preservation leaves the work in place. Preserving work neither resumes it nor makes it verified. **Enforced for coordinator cleanup**: `tests/test_arc682_recovery.py::TestARC682Recovery.test_000_review_failed_tracked_and_untracked_bytes_survive_cleanup` runs real Git teardown after a failed review and restores identical tracked and untracked bytes. Companion cases pin preservation-failure retention, late-write refusal, and exact `.git` pointer preservation. Legacy migration remains bounded: exhausted cleanup skips annotation, ID-only records can be marked absent without lookup, and path-only records without an owner or workspace ID lack migration; no annotation recovers previously deleted bytes.
